...
export default function useAppState() {
  ...
  const selectNote = useCallback(
    async (noteId: string | null) => {
      ...
      const meta = [...state.notes, ...state.trashNotes].find(
        (note) => note.id === noteId
      );
      if (!meta) {
        return;
    ...
    async (id: string) => {
      try {
        ...
        setState((prev) => ({
          ...
          trashNotes: prev.trashNotes.filter(
            (note) => note.notebookId !== id && note.originalNotebookId !== id
          ),
          selectedNoteId:
  ...
  const deleteNote = useCallback(async (notebookId: string, noteId: string) => {
    try {
      ...
      const trashedMeta = state.notes.find((note) => note.id === noteId);
      setState((prev) => ({
        ...prev,
        notes: prev.notes.filter((note) => note.id !== noteId),
        trashNotes: trashedMeta
          ? sortNotes([
              {
                ...trashedMeta,
                notebookId: '.trash',
  ...
  const restoreNote = useCallback(async (noteId: string) => {
    try {
      const restored = await window.desktopApi.restoreNote(noteId);
      const meta = {
        id: restored.id,
        title: restored.title,
        notebookId: restored.notebookId,
        originalNotebookId: restored.originalNotebookId,
        tags: restored.tags,
      ...
      setState((prev) => ({
        ...prev,
        notes: sortNotes([meta, ...prev.notes.filter((note) => note.id !== noteId)]),
        trashNotes: prev.trashNotes.filter((note) => note.id !== noteId),
        selectedNoteId: noteId,
        activeNote: restored,
